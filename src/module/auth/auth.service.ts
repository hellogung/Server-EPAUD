...
export class AuthService {
    ...
    async login(identifier: string, password: string): Promise<LoginResult> {
        ...
        if (!user) throw new HTTPException(401, { message: "Kredensial tidak valid" })

        const isValid = await Bun.password.verify(password, user.password)
        if (!isValid) throw new HTTPException(401, { message: "Kredensial tidak valid" })

        if (!user.is_verified) throw new HTTPException(403, { message: "Akun belum terverifikasi" })
    ...
    async refreshToken(token: string): Promise<string> {
        const payload = await verify(token, Config.REFRESH_SECRET_KEY as string)

        const redis = await RedisClient.getInstance()
        const savedToken = await redis.get(`refresh:${payload.sub}`)
        if (savedToken !== token) throw new HTTPException(401, { message: "Token tidak valid" })

        const user = await this.repo.findById(payload.sub as string)
        if (!user) throw new HTTPException(404, { message: "User tidak ditemukan" })

        return await JWTHelper.GenerateAccessToken(token, {
            username: user.username,
            role: user.role,
            full_name: user.full_name
        })
